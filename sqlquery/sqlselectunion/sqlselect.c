...
// 전체 출력
	...
	strcpy(query, "select A.no, A.name, A.phone, B.grade from person A left outer join grade B on A.name = B.name union select A.no, A.name, A.phone, B.grade from person A right outer join grade B on A.name = B.name");
